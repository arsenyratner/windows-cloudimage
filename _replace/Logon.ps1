...
try {
    ...
    try {
        $installCloudbaseInit = Get-IniFileValue -Path $configIniPath -Section "cloudbase_init" -Key "install_cloudbase_init" `
            -Default "True"
    } catch{}
	try {
    ...
    if ($installCloudbaseInit -and $installCloudbaseInit -ne 'False') {
