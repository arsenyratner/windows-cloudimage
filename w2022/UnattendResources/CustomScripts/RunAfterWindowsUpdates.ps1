...
Start-Process -Wait -NoNewWindow -FilePath "$ENV:SystemDrive\UnattendResources\CustomResources\virtio-win-guest-tools.0.1.266.exe" -ArgumentList "/s /qn"
...
Start-Process -Wait -NoNewWindow -FilePath "C:\ProgramData\chocolatey\choco.exe" -ArgumentList "install -y 7zip notepadplusplus bind-toolsonly git wget gawk grep gnuwin32-coreutils.install winscp putty.install sysinternals"
